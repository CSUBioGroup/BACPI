...
class BiDACPI(nn.Module):
    def __init__(self, task, n_atom, n_amino, params):
        ...
        self.U = [nn.Parameter(torch.empty(size=(latent_dim, latent_dim))) for _ in range(self.bidat_num)]
    ...
    def bidirectional_attention_prediction(self, atoms_vector, atoms_mask, fps, amino_vector, amino_mask):
        ...
        # atoms_vector = F.leaky_relu(self.c_final(atoms_vector), 0.1)
        # amino_vector = F.leaky_relu(self.p_final(amino_vector), 0.1)
        # fps = F.leaky_relu(self.super_final(fps), 0.1)
        for i in range(self.bidat_num):
            # print(atoms_vector.shape, self.U[i].shape)
            # tmp = torch.matmul(atoms_vector, self.U[i])
            # print(tmp.shape)
            # A = torch.tanh(torch.matmul(tmp, amino_vector.transpose(1, 2)))
