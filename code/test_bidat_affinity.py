...
class BiDACPI(nn.Module):
    def __init__(self, task, n_atom, n_amino, params):
        ...
        self.biatt_c = nn.ModuleList([nn.Linear(latent_dim, 1) for _ in range(self.bidat_num)])
        self.biatt_p = nn.ModuleList([nn.Linear(latent_dim, 1) for _ in range(self.bidat_num)])
    ...
    def bidirectional_attention_prediction(self, atoms_vector, atoms_mask, fps, amino_vector, amino_mask):
        ...
        for i in range(self.bidat_num):
            ...
            A_mask = torch.matmul(atoms_mask.view(b, -1, 1), amino_mask.view(b, 1, -1))
            A = A * A_mask
            ...
            atoms_tmp = torch.tanh(self.bihidden_c[i](atoms_vector)) * atoms_trans
            amino_tmp = torch.tanh(self.bihidden_p[i](amino_vector)) * amino_trans
...
def train_eval(model, task, data_train, data_dev, data_test, device, params):
    ...
    print2file('gat+cnn+monn', prefix + 'BACPI_IC50_log.txt', True)
    ...
    for epoch in range(params.num_epochs):
        ...
        # rmse, pearson, spearman = test(model, task, ER_data, params.batch_size, device)
        # print('ER rmse:{}, pearson:{}, spearman:{}'.format(rmse, pearson, spearman))
        #
        # rmse, pearson, spearman = test(model, task, channel_data, params.batch_size, device)
        # print('IC rmse:{}, pearson:{}, spearman:{}'.format(rmse, pearson, spearman))
        #
        # rmse, pearson, spearman = test(model, task, GPCR_data, params.batch_size, device)
        # print('GPCR rmse:{}, pearson:{}, spearman:{}'.format(rmse, pearson, spearman))
        #
        # rmse, pearson, spearman = test(model, task, kinase_data, params.batch_size, device)
        # print('kinase rmse:{}, pearson:{}, spearman:{}'.format(rmse, pearson, spearman))
...
if __name__ == '__main__':
    ...
    ER_data = load_data(data_dir, 'ER')
    GPCR_data = load_data(data_dir, 'GPCR')
    kinase_data = load_data(data_dir, 'kinase')
    channel_data = load_data(data_dir, 'channel')

    # data_train, data_dev = split_data(train_data, 0.1)
    ...
    model.to(device)
    train_eval(model, task, train_data, None, test_data, device, params)
    ...
    #
    # print('print results...')
    # rmse, pearson, spearman = test(best_model, task, train_data, params.batch_size, device)
    # print('Train rmse:{}, pearson:{}, spearman:{}'.format(rmse, pearson, spearman))
    #
    # rmse, pearson, spearman = test(best_model, task, test_data, params.batch_size, device)
    # print('Test rmse:{}, pearson:{}, spearman:{}'.format(rmse, pearson, spearman))
    #
    # rmse, pearson, spearman = test(best_model, task, ER_data, params.batch_size, device)
    # print('ER rmse:{}, pearson:{}, spearman:{}'.format(rmse, pearson, spearman))
    #
    # rmse, pearson, spearman = test(best_model, task, GPCR_data, params.batch_size, device)
    # print('GPCR rmse:{}, pearson:{}, spearman:{}'.format(rmse, pearson, spearman))
    #
    # rmse, pearson, spearman = test(best_model, task, kinase_data, params.batch_size, device)
    # print('kinase rmse:{}, pearson:{}, spearman:{}'.format(rmse, pearson, spearman))
    #
    # rmse, pearson, spearman = test(best_model, task, channel_data, params.batch_size, device)
    # print('channel rmse:{}, pearson:{}, spearman:{}'.format(rmse, pearson, spearman))