...
class BiDACPI(nn.Module):
    def __init__(self, task, n_atom, n_amino, n_go, params):
        ...
        self.embedding_layer_go = nn.Embedding(n_go+1, latent_dim)
        ...
        self.go0 = nn.Parameter(torch.empty(size=(latent_dim, latent_dim)))
        nn.init.xavier_uniform_(self.go0, gain=1.414)
        self.go1 = nn.Parameter(torch.empty(size=(latent_dim, latent_dim)))
        nn.init.xavier_uniform_(self.go1, gain=1.414)
        ...
        if task == 'affinity':
            self.output = nn.Linear(latent_dim * 2 * latent_dim * 2, 1)
        elif task == 'interaction':
            self.output = nn.Linear(latent_dim * 2 * latent_dim * 2, 2)
    ...
    def bidirectional_attention_prediction(self, atoms_vector, atoms_mask, fps, amino_vector, amino_mask, go_vector):
        ...
        pf_final = torch.cat([self.comb_p(cat_pf).view(b, -1), go_vector.view(b, -1)], dim=1)
        cf_pf = F.leaky_relu(torch.matmul(cf_final.view(b, -1, 1), pf_final.view(b, 1, -1)).view(b, -1), 0.1)
        return self.output(cf_pf)

    def forward(self, atoms, atoms_mask, adjacency, amino, amino_mask, fps, gos, gos_mask):
        ...
        go_feature = self.embedding_layer_go(gos)
        go_feature = F.leaky_relu(torch.matmul(go_feature, self.go0), 0.1)
        go_feature = F.leaky_relu(torch.matmul(go_feature, self.go1), 0.1)
        go_feature = torch.sum(go_feature * gos_mask.view(gos_mask.shape[0], -1, 1), dim=1) / torch.sum(gos_mask, dim=1, keepdim=True)

        prediction = self.bidirectional_attention_prediction(atoms_vector, atoms_mask,
                                                             super_feature, amino_vector, amino_mask, go_feature)
        return prediction
...
def train_eval(model, task, data_train, data_dev, data_test, device, params):
    ...
    for epoch in range(params.num_epochs):
        ...
        for i in range(math.ceil(len(data_train[0]) / batch_size)):
            ...
            atoms_pad, atoms_mask, adjacencies_pad, batch_fps, amino_pad, amino_mask, go_pad, go_mask, label = batch2tensor(batch_data, device)
            pred = model(atoms_pad, atoms_mask, adjacencies_pad, amino_pad, amino_mask, batch_fps, go_pad, go_mask)
            if task == 'affinity':
        ...
        if task == 'affinity':
            ...
            info = 'Train auc:{}, acc:{}, aupr:{}'.format(auc_train, acc_train, aupr_train)
            print(info)
            ...
            # auc_test, acc_test, aupr_test = test(model, task, data_sec_sep, batch_size, device)
            # info = 'seenChem_seenProt auc:{}, acc:{}, aupr:{}'.format(auc_test, acc_test, aupr_test)
            # print(info)
            #
            # auc_test, acc_test, aupr_test = test(model, task, data_usc_sep, batch_size, device)
            # info = 'unseenChem_seenProt auc:{}, acc:{}, aupr:{}'.format(auc_test, acc_test, aupr_test)
            # print(info)
            #
            # auc_test, acc_test, aupr_test = test(model, task, data_sec_usp, batch_size, device)
            # info = 'seenChem_unseenProt auc:{}, acc:{}, aupr:{}'.format(auc_test, acc_test, aupr_test)
            # print(info)
            #
            # auc_test, acc_test, aupr_test = test(model, task, data_usc_usp, batch_size, device)
            # info = 'unseenChem_unseenProt auc:{}, acc:{}, aupr:{}'.format(auc_test, acc_test, aupr_test)
            # print(info)

            auc_test, acc_test, aupr_test = test(model, task, data_seenprot, batch_size, device)
            info = 'seenProt auc:{}, acc:{}, aupr:{}'.format(auc_test, acc_test, aupr_test)
            print(info)

            auc_test, acc_test, aupr_test = test(model, task, data_unseenprot, batch_size, device)
            info = 'unseenProt auc:{}, acc:{}, aupr:{}'.format(auc_test, acc_test, aupr_test)
            print(info)
...
if __name__ == '__main__':
    ...
    data_dev = load_data(data_dir, 'dev')
    # data_sec_sep = load_data(data_dir, 'seenChem_seenProt')
    # data_usc_sep = load_data(data_dir, 'unseenChem_seenProt')
    # data_sec_usp = load_data(data_dir, 'seenChem_unseenProt')
    # data_usc_usp = load_data(data_dir, 'unseenChem_unseenProt')
    data_test = load_data(data_dir, 'test')

    data_seenprot = load_data(data_dir, 'seenProt')
    data_unseenprot = load_data(data_dir, 'unseenProt')

    atom_dict = pickle.load(open(data_dir + '/atom_dict', 'rb'))
    amino_dict = pickle.load(open(data_dir + '/amino_dict', 'rb'))

    print('training...')
    model = BiDACPI(task, len(atom_dict), len(amino_dict), 9616, params)
    model.to(device)
    train_eval(model, task, data_train, data_dev, data_test, device, params)
