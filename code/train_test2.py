...
class BiDACPI(nn.Module):
    def __init__(self, task, n_atom, n_amino, params):
        ...
        self.U = nn.Parameter(torch.empty(size=(comp_dim, comp_dim)))
    ...
    def Pairwise_pred_module(self, batch_size, comp_feature, prot_feature, vertex_mask, seq_mask):
        ...
        pairwise_pred = torch.tanh(torch.matmul(torch.matmul(pairwise_c_feature, self.U), pairwise_p_feature.transpose(1, 2)))
    ...
    def Affinity_pred_module(self, batch_size, comp_feature, prot_feature, super_feature, vertex_mask, seq_mask,
        ...
        # cat = torch.cat([cf.view(batch_size, -1), super_feature.view(batch_size, -1), pf.view(batch_size, -1)], dim=1)
        # for i in range(self.layer_out):
        #     cat = F.leaky_relu(self.fully_connect[i](cat), self.alpha)
        # prediction = self.predict(cat)
        # return prediction
    ...
    def biatt(self, atoms_vector, atoms_mask, amino_vector, amino_mask):
        pass
    ...
    def forward(self, atoms, atoms_mask, adjacency, amino, amino_mask, fps):
        ...
        # super_feature = F.leaky_relu(self.fps_first(fps), 0.1)
        # for i in range(2):
        #     super_feature = F.leaky_relu(self.fps_layer[i](super_feature), 0.1)
...
def train_eval(model, task, train_data, valid_data, test_data, device, params):
    ...
    for epoch in range(params.num_epochs):
        ...
        for i in range(math.ceil(len(train_data[0]) / batch_size)):
            batch_data = [train_data[di][idx[i * batch_size: (i + 1) * batch_size]] for di in range(len(train_data))]
...
def test(model, task, test_data, batch_size, device):
    ...
    for i in range(math.ceil(len(test_data[0]) / batch_size)):
        batch_data = [test_data[di][i * batch_size: (i + 1) * batch_size] for di in range(len(test_data))]
        atoms_pad, atoms_mask, adjacencies_pad, batch_fps, amino_pad, amino_mask, label = batch2tensor(batch_data, device)
        with torch.no_grad():
            pred = model(atoms_pad, atoms_mask, adjacencies_pad, amino_pad, amino_mask, batch_fps)
        if task == 'affinity':
...
if __name__ == '__main__':
    ...
    dir_input = '../datasets/' + DATASET + '/' + measure + '/'
    train_dir_input = dir_input + 'train/'
    test_dir_input = dir_input + 'test/'
    ...
    print('Load data...')
    train_compounds = np.load(train_dir_input + 'compounds.npy', allow_pickle=True)
    train_adjacencies = np.load(train_dir_input + 'adjacencies.npy', allow_pickle=True)
    train_fingerprint = np.load(train_dir_input + 'fingerprint.npy', allow_pickle=True)
    train_proteins = np.load(train_dir_input + 'proteins.npy', allow_pickle=True)
    train_interactions = np.load(train_dir_input + 'interactions.npy', allow_pickle=True)
    train_data = [train_compounds, train_adjacencies, train_fingerprint, train_proteins, train_interactions]

    test_compounds = np.load(test_dir_input + 'compounds.npy', allow_pickle=True)
    test_adjacencies = np.load(test_dir_input + 'adjacencies.npy', allow_pickle=True)
    test_fingerprint = np.load(test_dir_input + 'fingerprint.npy', allow_pickle=True)
    test_proteins = np.load(test_dir_input + 'proteins.npy', allow_pickle=True)
    test_interactions = np.load(test_dir_input + 'interactions.npy', allow_pickle=True)
    test_data = [test_compounds, test_adjacencies, test_fingerprint, test_proteins, test_interactions]
