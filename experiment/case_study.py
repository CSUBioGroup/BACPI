...
def train_eval(model, task, data_train, data_dev, data_test, device, params):
    ...
    for epoch in range(params.num_epochs):
        ...
        rmse_dev, pearson_dev, spearman_dev, _ = test(model, task, data_dev, batch_size, device)
        info = 'Dev rmse:{}, pearson:{}, spearman:{}'.format(rmse_dev, pearson_dev, spearman_dev)
        print(info)

        # rmse_test, pearson_test, spearman_test, case_pred = test(model, task, data_test, batch_size, device)
        # info = 'Test rmse:{}, pearson:{}, spearman:{}'.format(rmse_test, pearson_test, spearman_test)
        # print(info)

        if rmse_dev < min_rmse_dev:
            min_rmse_dev = rmse_dev
            torch.save(model, '../checkpoint/ic50_case_study3.pt')
...
def main():
    ...
    val_data = load_data(data_dir, 'test')
    ...
    model.to(device)
    train_eval(model, task, train_data, val_data, None, device, params)
    ...
    #     cid_pred_top100 = sorted(cid_pred, key=lambda item: item[1], reverse=True)[:100]
    #     pred_top100 = pd.DataFrame(cid_pred_top100, index=None)
    #     pred_top100.to_csv('../results/case_study/cids_top100_' + case + '_2.csv', index=False, header=['CID', 'affinity'])
