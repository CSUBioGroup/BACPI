...
def process_interaction(file_name, dir_input):
    ...
    compounds, adjacencies, fps, proteins, interactions = [], [], [], [], []
    compound_protein_pair = []

    for no, data in enumerate(data_list):
        print('/'.join(map(str, [no + 1, N])))

        cid, pid, smiles, sequence, interaction = data.strip().split()

        compound_protein_pair.append((int(cid), str(pid)))
    ...
    np.save(dir_input + 'proteins', proteins)
    np.save(dir_input + 'interactions', interactions)
    np.save(dir_input + 'sample_ids', compound_protein_pair)
...
if __name__ == "__main__":
    ...
    # filename = '../data/' + DATASET + '/' + ratio + '/data_id.txt'
    # dirinput = '../datasets/' + DATASET + '/' + ratio + '/'
    # process_interaction(filename, dirinput)
    # dump_dictionary(fingerprint_dict, dirinput + 'atom_dict')
    # dump_dictionary(word_dict, dirinput + 'amino_dict')
